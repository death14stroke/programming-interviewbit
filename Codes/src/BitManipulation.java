...
public class BitManipulation {
    ...
    static int minXor(int[] A) {
        ...
        Trie trie = new Trie();
        trie.insert(A[0]);
        // search for the number with the largest number of similar bits with current number till now
        // take xor and compare with minimum. Insert current number in trie
        for (int i = 1; i < A.length; i++) {
            minXor = Math.min(minXor, minXorUtil(trie.root, A[i]));
            trie.insert(A[i]);
        }
    ...
    // util to find the number with the largest number of bits similar to key in trie
    // and take xor with it as the result xor will be minimum
    private static int minXorUtil(TrieNode root, int key) {
        TrieNode curr = root;
    ...
    static int palindromicBinary(int A) {
        // base case
        if (A == 1)
            return 1;
